...
class PositionalEncodingLayer(nn.Module):
    ...
    def forward(self, feat):
        ...
        inv_freq_x = math.pi / (x - 1)
        inv_freq_y = math.pi / (y - 1)
        inv_freq_z = math.pi / (z - 1)
...
class CoTr(nn.Module):
    ...
    def forward(self, q, k):
        ...
        q = q.reshape(B, N, C, 1).transpose(2, 3)   # B, N, 1, C
        ...
        k = k.permute(0, 3, 2, 1)  # (B, N, num_tokens, C)

        attn = (q @ k.transpose(-2, -1))  # B x N x 1 x num_tokens
        attn = attn.softmax(dim=-1)

        v = self.makeV(N) # B, N, num_tokens(self.kernel_size**3), 3
        x = (attn @ v)  # B x N x 1 x 3
