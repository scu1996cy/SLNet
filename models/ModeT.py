...
class ModeTransformer(nn.Module):
    ...
    def forward(self, q, k):
        ...
        k = k.permute(0, 4, 1, 3, 2)  # (1, N, heads, num_tokens, head_dim)
...
class ModeT(nn.Module):
    def __init__(self,
        ...
        self.peblock1 = PositionalEncodingLayer(2*c, dim=head_dim*num_heads[4]) # 2*4=8, 6*1=6
        self.mdt1 = ModeTransformer(head_dim*num_heads[4], num_heads[4], qk_scale=scale)

        self.peblock2 = PositionalEncodingLayer(4*c, dim=head_dim*num_heads[3]) # 4*4=16, 6*1=6
        self.mdt2 = ModeTransformer(head_dim*num_heads[3], num_heads[3], qk_scale=scale)

        self.peblock3 = PositionalEncodingLayer(8*c, dim=head_dim*num_heads[2]) # 8*4=32, 6*2=12
        self.mdt3 = ModeTransformer(head_dim*num_heads[2], num_heads[2], qk_scale=scale)
        self.cwm3 = CWM(3 * num_heads[2], 3 * num_heads[2] * 2)

        self.peblock4 = PositionalEncodingLayer(16*c, dim=head_dim*num_heads[1]) # 16*4=64, 6*4=24
        self.mdt4 = ModeTransformer(head_dim*num_heads[1], num_heads[1], qk_scale=scale)
        self.cwm4 = CWM(3 * num_heads[1], 3 * num_heads[1] * 2)

        self.peblock5 = PositionalEncodingLayer(32*c, dim=head_dim*num_heads[0]) # 32*4=128, 6*8=48
